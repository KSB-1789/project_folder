...
const initializeEventListeners = () => {
    ...
    document.body.addEventListener('click', (e) => {
        ...
        if (deleteBtn) TimetableManager.delete(deleteBtn.dataset.id);
        if (duplicateBtn) {
        ...
        if (clearBtn) {
            showCustomConfirm("Are you sure? This resets all attendance but keeps your timetables.").then(confirmed => {
                if (confirmed) {
                    showLoading('Clearing records...');
                    supabase.from('attendance_log').delete().eq('user_id', appState.currentUser.id).then(() => {
                        DataManager.saveUserProfile({ ...appState.userProfile, last_log_date: null }).then(() => window.location.reload());
                    });
                }
            });
        }
        ...
        if (e.target.id === 'save-attendance-btn') {
            DataManager.saveAttendanceChanges().then(() => runFullAttendanceUpdate());
        }
